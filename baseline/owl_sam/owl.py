...
def _detect_once(image_pil: Image.Image, labels: List[str], model, processor, device):
    # `labels` here are the text prompts fed to OWLv2.
...
def _pool_candidates(labels: List[str], per_boxes, per_scores, per_lids) -> Dict[str, Tuple[np.ndarray, np.ndarray]]:
    """
    `labels` are the logical keys (component names), aligned with class indices.
    """
    ...
    for B, S, L in zip(per_boxes, per_scores, per_lids):
        for i, u in enumerate(labels):
            sel = (L == i)
            if np.any(sel):
                pooled[u][0].append(B[sel])
                pooled[u][1].append(S[sel])
    out: Dict[str, Tuple[np.ndarray, np.ndarray]] = {}
    for u in labels:
        if pooled[u][0]:
            out[u] = (
                np.concatenate(pooled[u][0], 0),
                np.concatenate(pooled[u][1], 0),
            )
        else:
            out[u] = (
...
def _select_counts_no_overlap(
    ...
    """
    Greedy global selection; results keyed by item["name"].
    """
    ...
    for name, (B, S) in pooled.items():
        ...
        chosenB = np.empty((0, 4), np.float32)
        chosenS = np.empty((0,), np.float32)
...
def detect_owlv2_boxes_counts(
    ...
    """
    Uses OWLv2 for zero-shot detection.
    - If an item has a "query" field, that text is used as the prompt.
      e.g. name="door", query="door of a car"
    - Results remain keyed by item["name"].
    """

    # --- logical names & corresponding text prompts ---
    names: List[str] = []
    ...
    # --- full image pass (prompts = label_texts) ---
    b_full, s_full, lid_full = _detect_once(image_pil, label_texts, model, processor, device)
    per_boxes = [b_full]
    per_scores = [s_full]
    per_lids = [lid_full]

    # --- tiles (same label_texts order => same class indices) ---
    if use_tiles:
    ...
    # --- pool by logical names (indices aligned with label_texts) ---
    pooled = _pool_candidates(names, per_boxes, per_scores, per_lids)

    # --- per-item selection keyed by name ---
...
if __name__ == "__main__":
    ...
    res = detect_owlv2_boxes_counts(
        image_pil=img,
        items=items,  # no query here; falls back to names
    ...
    for it in items:
        ...
        for i, b in enumerate(boxes):
            ...
            cv2.putText(bgr, txt, (x1, max(0, y1 - 4)),
